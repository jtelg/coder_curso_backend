...
if (location.pathname === "/") {
  ...
  const renderChat = (data) => {
    console.log();
    const denormData = new normalizr.denormalize(
      data.result,
      [data],
      data.entities
    );
    const initVal = JSON.stringify(data).length;
    const finVal = JSON.stringify(denormData).length;
    const res = (((initVal / finVal) * 100) - 100);
    document.getElementById('porcentaje').innerText = `%${res.toLocaleString('de')}`;
    const html = `
      <div id="overf" class="flex flex-col gap-1 absolute bottom-0 max-h-[19.3rem] overflow-auto w-full">
        ${denormData
          .map((x) => {
            return `
            <p> <span class="text-[${x.author.color}] text-[14px] font-bold">${x.author.nombre}</span> <span class="text-[11px]">[${x.feccarga}]</span> : <span class="text-[14px] font-bold text-green-600">${x.text}</span> </p>
      `;
          })
          .join(" ")}
      </div>
