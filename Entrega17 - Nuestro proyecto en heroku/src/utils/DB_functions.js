...
class Contenedor {
  ...
  async save(obj) {
    try {
      ...
      logger.error(`Error en el POST en ${this.tablename} (${this._DB}) - ${error}`)
    }

  }
  async getById(id) {
    try {
      logger.info(`GET por ID en ${this.tablename} (${this._DB})`);
      if (this._DB !== "JSON") {
      ...
      logger.error(`Error al buscar por ID en ${this.tablename} (${this._DB}) - ${error}`)
      return null;
    }
  ...
  async getAll() {
    logger.info(`GET ALL en ${this.tablename} (${this._DB})`);
    try {
      switch (this._DB) {
        case "SqliteDB":
      ...
      logger.error(`Error al buscar listado en ${this.tablename} (${this._DB}) - ${error}`)
      return [];
    }
  ...
  async getxCampo(name, value) {
    logger.info(`GET x campo en ${this.tablename} (${this._DB})`);
    try {
      switch (this._DB) {
        case "SqliteDB":
          return await knex_sqliteDB(this.tablename).select('*').where({email: name});
        case "MariaDB":
          return await knex_mariaDB(this.tablename).select('*').where({email: name});
        case "JSON":
        case "Firebase":
      ...
      logger.error(`Error al buscar x campo en ${this.tablename} (${this._DB}) - ${error}`)
      return null;
    }
  ...
  async getUserLogin(email, password) {
    ...
    try {
      ...
      logger.error(`GET user login (${this._DB}) - ${error}`)
      return null;
    }
  ...
  async deleteById(id) {
    logger.info(`DELETE x ID en ${this.tablename} (${this._DB})`);
    try {
      switch (this._DB) {
        case "SqliteDB":
      ...
      logger.error(`Error al eliminar por ID en ${this.tablename} (${this._DB}) - ${error}`)
    }
 
  }

  deleteAll() {
    logger.info(`DELETE ALL en ${this.tablename} (${this._DB})`);
    try {
      this.escribirArchivo("");
    } catch (error) {
      logger.error(`Error al eliminar todo en ${this.tablename} (${this._DB}) - ${error}`)
    }
  ...
  escribirArchivo(dato) {
    ...
    try {
      ...
      logger.error(`Error al escribir archivo (${this._DB}) - ${error}`)
    }
  ...
  async saveProds_xcarro(idcarro, prod) {
    logger.info(`SAVE x carro en ${this.tablename} (${this._DB})`);
    try {
      ...
      logger.error(`Save x carro ${this.tablename} (${this._DB}) - ${error}`)
    }
  ...
  async deleteProd_xcarro(idcarro, id_prod) {
    ...
    try {
      ...
      logger.error(`DETELE PROD x carro ${this.tablename} (${this._DB}) - ${error}`)
    }
